...
const { detectSpam } = require("../services/spamDetector"); // ✅ NEW IMPORT
...
module.exports.getReviewData = async (req, res) => {
  try {
    const allReview = await Review.find({ isSpam: { $ne: true } }) // ✅ HIDE SPAM FROM FEED
      .sort({ createdAt: -1 })
      .populate("user", "name")
      .lean();
...
module.exports.addReview = async (req, res) => {
  try {
    ...
    const savedReview = await newReview.save(); // ✅ SAVE FIRST

    // ✅ SPAM DETECTION — runs in background, does NOT slow down response
    setImmediate(async () => {
      try {
        const result = await detectSpam(savedReview, req.user, Review);
        ...
        if (result.isSpam) {
          console.log(
            `⚠️ Spam detected in review [${savedReview._id}] Score: ${result.spamScore}`,
            result.spamReasons
          );
        } else {
          console.log(`✅ Review [${savedReview._id}] passed spam check. Score: ${result.spamScore}`);
        }
      } catch (err) {
    ...
    // ✅ RESPOND IMMEDIATELY — don't wait for spam check
    res.status(201).json({
      success: true,
...
module.exports.searchReviews = async (req, res) => {
  try {
    ...
    let filter = { isSpam: { $ne: true } }; // ✅ EXCLUDE SPAM FROM SEARCH RESULTS
...
module.exports.getSimilarReviews = async (req, res) => {
  try {
    ...
    let similarReviews = await Review.find({
      _id: { $ne: id },
      isSpam: { $ne: true }, // ✅ EXCLUDE SPAM FROM SIMILAR REVIEWS
    ...
    if (similarReviews.length < 3) {
      const moreReviews = await Review.find({
        _id: { $ne: id },
        isSpam: { $ne: true }, // ✅ EXCLUDE SPAM FROM FALLBACK TOO
        $or: [
          { location: currentReview.location },
          { roomType: currentReview.roomType },
          { priceRange: currentReview.priceRange },
...
module.exports.getMyReviews = async (req, res) => {
  try {
    ...
    const userId = req.user.id;
    console.log(userId);
    // ✅ Show user their OWN reviews (including spam flagged ones so they know)
